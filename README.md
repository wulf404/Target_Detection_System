...
| `kDeepStreamStopOnCudaError` | Останавливать pipeline после фатальной CUDA/TensorRT ошибки вместо бесполезного повторного открытия камеры. |
...
Engine нужно заново собрать непосредственно на целевом Jetson Orin NX с той же
версией TensorRT, затем перезапустить приложение.
