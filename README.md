...
| `kDeepStreamOnnxPath` | Исходная ONNX-модель `1280x1280`, используемая только для первой сборки DeepStream engine на целевом Jetson. |
| `kDeepStreamEnginePath` | Отдельный TensorRT engine для `nvinfer`. Сейчас `/home/nick/qt/yolo_quadro_weights/quadron_1280_fp16_ds_jp621_orin_nx.engine`. |
| `kDeepStreamNetworkInputWidth`, `kDeepStreamNetworkInputHeight` | Входной размер engine. Сейчас `1280x1280`. |
| `kDeepStreamPreferH264Capture` | Сначала пробовать H.264 поток камеры вместо MJPEG. Для `HDMI USB Camera` в режиме 4K60 сейчас `true`. |
| `kDeepStreamScaleInMuxToNetworkInput` | Делать direct resize до `1280x1280` уже в `nvstreammux`, снижая нагрузку на путь перед `nvinfer`. |
| `kDeepStreamStopOnNvinferError` | Останавливать pipeline после фатальной ошибки `nvinfer`/TensorRT вместо бесполезного повторного открытия камеры. |
...
```text
v4l2src (H264 first, MJPEG fallback) -> nvv4l2decoder
  -> nvstreammux direct resize 1280x1280 -> nvinfer -> appsink
```

`nvinfer` использует отдельный целевой TensorRT engine
`quadron_1280_fp16_ds_jp621_orin_nx.engine`. Если этого файла еще нет, при
первом запуске он автоматически строится на текущем Jetson из
`quadron_1280.onnx`; это может занять несколько минут. После этого инференс
снова работает через сохраненный engine, а не через ONNX.
...
Если в логе появляется предупреждение TensorRT
`Using an engine plan file across different models of devices`, этот engine
собран не для текущего Jetson/GPU. После него возможен
`cudaErrorIllegalAddress`; повторное открытие камеры это не исправляет.
Не копируйте engine с другой машины и не подставляйте `quadron_2560_fp16.engine`
при входе сети `1280x1280`. При появлении такого предупреждения удалите только
файл `quadron_1280_fp16_ds_jp621_orin_nx.engine`, чтобы приложение заново
построило его на целевом Jetson, затем перезапустите приложение.
